...
standartizedData = scaler.transform(df)
standartizedData = pd.DataFrame(standartizedData, index = df.index, columns=df.columns)
Sdata_y = standartizedData[y_column].values
Sdata_y = Sdata_y.astype('int')

# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\  QUESTİON _ 3 ///////////////////////
corr = standartizedData.corr()["FFMC"].abs().sort_values(ascending=False)
print(corr)

# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\  QUESTİON _ 4_5_6 ///////////////////////
highest_corr = standartizedData.corr()["FFMC"].abs().sort_values(ascending=False).nlargest(len(df.columns)//2)
print(highest_corr.index)
hig_data = standartizedData[highest_corr.index].iloc[:,1:]
hig_data = hig_data.values


kf = FoldCross(hig_data, Sdata_y)
labels = kf.index
R2 = kf.R2
MSE = kf.MSE
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\  QUESTİON _ 7 ///////////////////////
X_train, X_test, Y_train, Y_test = train_test_split(hig_data,Sdata_y,test_size=0.2, random_state=99)
regressionCode = ["l","R","L","E","K","D","RF","S"]
regname = ["Linear", "Ridge", "Lasso", "ElasticNet", "K-NN", "Decision Tree", "Random Forest", "SVR"]
i=0
for name in regressionCode:
